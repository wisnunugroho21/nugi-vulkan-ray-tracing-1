...
void main() {
  ...
  for (int i = 0; i < globalLight.numLight; i++) {
    ...
    HitRecord hit = hitBvh(r, 0.001, 1000000.0);
    vec3 lightColor = vec3(0.0);

    if (!hit.isHit) {
      hit = hitSphere(globalLight.pointLights[i].sphere, r, 0.001, 1000000.0);

      vec3 distance = (hit.point - r.origin) / 10.0;
      float brdf = brdfVal(NoV, NoL, NoH, VoH);
      float NloL = dot(hit.faceNormal.normal, normalize(r.direction));

      lightColor = areaLight(globalLight.pointLights[i].sphere) * globalLight.pointLights[i].color * brdf * NoL * NloL / dot(distance, distance);
    }
